...
def generate_instagram_reel(custom_topic=None):
    try:
        ...
        reel_created, r_err = build_reel_video(image_url, "voice.mp3", "reel.mp4", headline=chosen_topic, script_text=voiceover_text)
        if not reel_created:
        ...
        send_telegram_message(f"⚠️ Exception in Reel Generation: {str(e)[:200]}")
...
def generate_instagram_post(custom_topic=None):
    try:
        ...
        send_telegram_message(f"⚠️ Exception in Post Generation: {str(e)[:200]}")
...
def reply_to_comments():
    try:
        ...
        send_telegram_message(f"⚠️ Exception in Comment Replies: {str(e)[:200]}")
